...
)
{
	...
	redr = handle_redirect(node->tokens[addr], expanded_filename[0]);
	ft_arrclear((void *)expanded_filename, free);
	out = ft_lstnew(redr);
	if (!redr || !out)
...
)
{
	...
	(void)addr;
	redr = handle_redirectfd(node->tokens[addr]);
	out = ft_lstnew(redr);
	if (!redr || !out)
		meta->error = AST_ERR_FATAL;
	if (!out)
...
)
{
	...
	redr = handle_heredoc(meta, node->tokens[addr + 1]->raw);
	out = ft_lstnew(redr);
	if (!out || !redr)
		meta->error = AST_ERR_FATAL;
	if (!out)
...
)
{
	...
	{
		new_node = _heredoc(meta, node, *inc);
		inc[0]++;
	}
	else if (arr[*inc]->type == TOK_REDIR_FD)
		new_node = _redirectfd(meta, node, *inc);
	else
	{
		new_node = _redirect(meta, node, *inc);
		inc[0]++;
	}
